...
// store port number locally upon initialisation.
...
int16_t* mic_allocateHeapBuffer(int sizeOfBuffer) 
{
    ...
    if (!buffer) {
        ...
        // err_log_allOutputs(TAG, "Failed to allocate memory in allocateHeapBuffer", NULL, NULL);
        return NULL;
    }
    memset(buffer, 0, sizeOfBuffer * sizeof(int16_t)); // set every array bucket to 0
    return buffer;
}
